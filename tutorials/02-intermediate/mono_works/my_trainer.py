...
#decoder and encode
    ...
    #mask encoder
        ...
        self.models["mask_encoder"] = networks.ResnetEncoder(
            num_layers=self.opt.num_layers,#18
            pretrained=False,#self.opt.weights_init == "pretrained",
            #encoder_path=self.opt.encoder_path,
            num_input_images=4
        )
        self.models["mask_encoder"].to(self.device)
        self.parameters_to_train += list(self.models["encoder"].parameters())

    #mask decoder

        self.models["mask"] = networks.DepthDecoder(
            num_ch_enc=self.models["mask_encoder"].num_ch_enc,
