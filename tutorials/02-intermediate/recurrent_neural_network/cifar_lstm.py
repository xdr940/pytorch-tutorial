...
class RNN(nn.Module):
    ...
    def forward(self, x):  # x [B,28,28]
        # Set initial hidden and cell states# b
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)  # [B,28,128]
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(device)  # [B,28,128]
...
for epoch in range(num_epochs):
    for i, (images, labels) in enumerate(train_loader):  # image [B,1,28,28]
       # images = images.reshape(-1, sequence_length, input_size).to(device)  # [b,28,28]
        ...
        loss = criterion(outputs, labels)  # 多分类
