...
parser = argparse.ArgumentParser(description='手推',
...
def make_mnist_cnn(X_dim, num_class):
    ...
    relu_conv = ReLU()
    maxpool = Maxpool(conv.out_dim, size=2, stride=1)#out_dim = [32, 28 ,28 ]
    flat = Flatten()
...
if __name__ == "__main__":
    ...
    if args.dataset =='mnist':
        ...
        training_set, test_set = load_mnist(
            '/home/roit/datasets/mnist/mnist.pkl.gz', num_training=1000, num_test=1000)
        X, y = training_set
        X_test, y_test = test_set
        mnist_dims = (1, 28, 28)# chw
        layers = make_mnist_cnn(mnist_dims, num_class=10)#definition layers
        cnn = CNN(layers)#connect layers
